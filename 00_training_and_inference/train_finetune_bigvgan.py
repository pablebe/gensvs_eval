...
import itertools
import os
os.environ["CUDA_VISIBLE_DEVICES"] = "2"#"0,1,2"#,2"
os.environ["WANDB_API_KEY"] = 'eecdd46f7d6d52ecaf6dff06f4a9f4025eff5f1a'
...
from sgmse.data_module import MSSSpecs
...
from evaluation.helper_functions import frame_and_win
...
#    )
    ...
    if rank == 0:
        ...
        #     logger = WandbLogger(project="sgmse-MSS", log_model=False, save_dir="logs", name=a.wandb_name, id=a.run_id, resume="allow")
        #     logger.experiment.log_code(".")
        #     wandb.init(project="sgmse-MSS", name=a.wandb_name, dir="logs", id=a.run_id, resume="must")
...
#                x, y, _, y_mel = batch
                ...
                #fuse batch and channel dimensions
#                audio_y = audio_y.reshape(audio_y.size(0)*audio_y.size(1), audio_y.size(2))
#                audio_x = audio_x.reshape(audio_x.size(0)*audio_x.size(1), audio_x.size(2))
#                import soundfile
#                soundfile.write('TEMP_OUT/mix.wav', audio_x.squeeze().cpu().numpy().T, h.sampling_rate)
#                soundfile.write('TEMP_OUT/targ.wav', audio_y.squeeze().cpu().numpy().T, h.sampling_rate)
                

#                stft_target = torch.stft(audio_y, **stft_kwargs).to(device)
                y = audio_y.to(device)
                x = audio_x.to(device)
                with torch.no_grad():
                    norm_fac = x.abs().max()
                ...
                #stft_target = stft_target.abs().unsqueeze(-1)#.to(device)
                    y_mel = mel_spectrogram(
                                            ...
                                            h.fmin,
                                            h.fmax_for_loss,
                                            )#einsum('b n f t c, f m -> b n m t c', stft_target, melroformer.mel_fb).squeeze(-1)

                    if hasattr(generator, "module"):
                        y_g_hat = generator.module(mel_sep)
                    else:
                        y_g_hat = generator(mel_sep)
    #                soundfile.write('TEMP_OUT/pred.wav', y_g_hat.squeeze().cpu().numpy().T, h.sampling_rate)
    ...
    #            logger.log(f"validation_{mode}/mrstft", val_mrstft, step=steps)
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        for i, batch in  tqdm(enumerate(train_loader), total=len(train_loader), desc='Finetunig BigVGAN, epoch: '+str(epoch)):
            ...
            seg_ct = 0
#            loss_gen_all_print = 0
#            mel_error_print = 0
            for sep_seg, targ_seg in zip(sep_audio_seg, y_seg):
                #temp_sep_audio = sep_seg.unsqueeze(0)
#                import soundfile
#                soundfile.write('TEMP_OUT/sep.wav', sep_seg.squeeze().cpu().numpy().T, h.sampling_rate)
#                soundfile.write('TEMP_OUT/targ.wav', targ_seg.squeeze().cpu().numpy().T, h.sampling_rate)
                seg_ct += 1
                mel_sep = mel_spectrogram(
                            sep_seg,
                            h.n_fft,
...
#                mel_sep = spectral_normalize_torch(mel_sep)
#                mel_sep = mel_sep.squeeze()

#            rand_ch = torch.randint(0, audio_y.size(1), (1,))
#            mel_sep = mel_sep[rand_ch,...]
            #y = y.to(device)
#            stft_target = stft_target[:,rand_ch,...]
#            mel_sep = mel_sep.reshape(mel_sep.size(0)*mel_sep.size(1), mel_sep.size(2), mel_sep.size(3)).to(device)
            #y = y.to(device)
#            stft_target = stft_target.abs().unsqueeze(-1)#.to(device)
#            y_mel = einsum('bn f t c, f m -> bn m t c', stft_target, melroformer.mel_fb).squeeze(-1)
            # mel_sep = mel_spectrogram(
            #             sep_audio.squeeze(),
            #             h.n_fft,
            #             h.num_mels,
...
#                loss_gen_all_print += loss_gen_all.item()
#                mel_error_print += loss_mel.item()

#                loss_gen_all_print /= seg_ct
#                mel_error_print /= seg_ct
#                mel_error_print = mel_error_print / lambda_melloss
                if rank == 0:
                    # STDOUT logging
                    ...
                    if steps % a.summary_interval == 0:
                        ...
                        # logger.log("training/mel_spec_error", mel_error, steps)
                        # logger.log("training/fm_loss_mpd", loss_fm_f.item(), steps)
                        # logger.log("training/gen_loss_mpd", loss_gen_f.item(), steps)
                        # logger.log("training/disc_loss_mpd", loss_disc_f.item(), steps)
                        # logger.log("training/grad_norm_mpd", grad_norm_mpd, steps)
                        # logger.log("training/fm_loss_mrd", loss_fm_s.item(), steps)
                        # logger.log("training/gen_loss_mrd", loss_gen_s.item(), steps)
                        # logger.log("training/disc_loss_mrd", loss_disc_s.item(), steps)
                        # logger.log("training/grad_norm_mrd", grad_norm_mrd, steps)
                        # logger.log("training/grad_norm_g", grad_norm_g, steps)
                        # logger.log(
                        ...
                        # )
                        # logger.log("training/epoch", epoch + 1, steps)
    ...
    #                    for i_x in range(x.shape[0]):
                            ...
                            #     steps,
                            # )
    #                        log_data_dict["file #"+str(i_x)] = [wandb.Audio(x[i_x], h.sampling_rate, caption="mixture"), wandb.Audio(y[i_x], h.sampling_rate, caption="target"), wandb.Audio(y_g_hat[i_x], h.sampling_rate, caption="separated")]
                            # sw.add_audio(
...
#    dataset_parser = parser.add_argument_group("dataset", description="Lightning Trainer")
